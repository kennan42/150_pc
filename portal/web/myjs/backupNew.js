...
//2.8_JIABAN个人考勤之30天内加班数据
function submit_2_8_JIABAN(BEGDA,ENDDA,pernr){
	var rtnJSON = {};
	if(pernr){
		//拼接提交参数
		var paramJSon = {
		...
		$.post("/zhrws/zhrws2221",JSON.stringify(paramJSon),function(pbData){
			var pbData = typeof pbData == 'object'?pbData:JSON.parse(pbData);
			if (pbData.ES_PUBLIC.TYPE == '0') {
				rtnJSON.status = "0";
				rtnJSON = pbData;
			}else {
				rtnJSON.status = "-1";
...
//2.8_JIABAN个人考勤之30天内加班数据
function submit_2_8_XIUJIA(pernr){
	var rtnJSON = {};
	if(pernr){
		//拼接提交参数
		var paramJSon = {
		...
		$.post("/zhrws/zhrws2220",JSON.stringify(paramJSon),function(pbData){
			var pbData = typeof pbData == 'object'?pbData:JSON.parse(pbData);
			if (pbData.ES_PUBLIC.TYPE == '0') {
				rtnJSON.status = "0";
				rtnJSON = pbData;
...
function submit_4_1_xin(p_obj){
	...
	if(p_obj){
		...
		$.post("/zbcm/zbcm4hr007",JSON.stringify(paramJSon),function(orgData){
			var orgData = typeof orgData == 'object'?orgData:JSON.parse(orgData);
			if (orgData.status == '-1') {
				rtnJSON.status = "-1";
			}
...
function submit_8_11(pernr){
    ...
    if(pernr){
        ...
        $.post("/zhrws/zhrws2119",JSON.stringify(paramJSon),function(nxjTimeData){
            var nxjTimeData = typeof nxjTimeData == 'object'?nxjTimeData:JSON.parse(nxjTimeData);
            if (nxjTimeData.status == '-1') {
                rtnJSON.status = "-1";
            }
            else {
                if(nxjTimeData.RETURN_SUBRC != '0'){
                    rtnJSON.status = "-1";
                }else{
                    rtnJSON = nxjTimeData;
                    rtnJSON.status = "0";
